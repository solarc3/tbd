  ...
  <div>
    <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-lg relative">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <!-- Campo de fecha vencimiento ahora a pantalla completa, no en grid -->
        <div class="mb-4">
          <label class="block text-sm font-medium mb-1 text-gray-700">Fecha de Vencimiento</label>
          <input
              v-model="localTask.fechaVencimiento"
              type="datetime-local"
              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 date-input"
              required
          >
        </div>
...
/* Aumenta el tamaño del input de fecha en dispositivos móviles */
...
/* Aumenta el ancho del modal para tener más espacio */
.modal-content {
  max-width: 32rem;
}
</style>
